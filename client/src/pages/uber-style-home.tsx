...
export default function UberStyleHome() {
  ...
  const getAddressSuggestions = async (query: string): Promise<
    Array<{
      id: string;
      name: string;
      address: string;
  ...
  const geocodeAddress = async (address: string): Promise<{ lat: number; lng: number } | null> => {
    try {
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address + ", Amsterdam, Netherlands")}&limit=1&addressdetails=1`,
        {
          headers: {
            "User-Agent": "Spoedpakketjes/1.0",
          },
  ...
  const calculateEstimate = (pickup?: { lat: number; lng: number }, delivery?: { lat: number; lng: number }) => {
    if (pickup && delivery) {
      ...
      const a =
        ...
        Math.cos((pickup.lat * Math.PI) / 180) * Math.cos((delivery.lat * Math.PI) / 180) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
      ...
      <div className="h-screen bg-white">
        ...
        <div className="bg-white p-4 border-b">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-lg font-semibold">{showLocationPicker === "pickup" ? "Ophaallocatie" : "Bezorglocatie"}</h1>
          </div>
        </div>
    ...
    <div className="h-screen bg-white relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <SimpleOsmMap
          ...
          driverLocation={pickup && delivery ? { lat: 52.37, lng: 4.895 } : undefined}
        ...
        <Card className="shadow-lg">
          <CardContent className="p-4 space-y-3">
            ...
            {!showServiceSelection && (
              <div className="mt-4">
                <Button className="w-full bg-brand-blue text-white py-4 h-auto text-lg font-semibold" onClick={handleCalculateEstimate} disabled={!pickup || !delivery}>
                  Berekenen
                </Button>
              </div>
            )}
      ...
      {showServiceSelection && (
        <div className="absolute bottom-0 left-0 right-0 z-30 bg-white rounded-t-3xl shadow-2xl max-h-[calc(100vh-5rem)] overflow-y-auto">
          <div className="p-6 pb-24">
            ...
            <div className="space-y-3 mb-6">
              {services.map((service) => (
                <Button
                  ...
                  className={`w-full h-auto p-4 justify-between ${selectedService === service.id ? "bg-brand-blue text-white" : "bg-gray-50"}`}
            ...
            <Button className="w-full bg-brand-blue text-white py-4 h-auto text-lg font-semibold" onClick={handleBookDelivery}>
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                {isCustomer && (
                  <>
                    <Button variant="outline" className="h-16 flex flex-col space-y-1 border-purple-200 hover:bg-purple-50" onClick={() => setLocation("/payment")}>
                      <div className="text-xl">💳</div>
                      <span className="text-xs">Betaling</span>
                    </Button>
                    <Button variant="outline" className="h-16 flex flex-col space-y-1 border-red-200 hover:bg-red-50" onClick={() => setLocation("/activity")}>
                      <div className="text-xl">📊</div>
                ...
                {isDriver && (
                  <>
                    <Button variant="outline" className="h-16 flex flex-col space-y-1 border-green-200 hover:bg-green-50" onClick={() => setLocation("/driver")}>
                      <div className="text-xl">👨‍💼</div>
                      <span className="text-xs">Dashboard</span>
                    </Button>
                    <Button variant="outline" className="h-16 flex flex-col space-y-1 border-orange-200 hover:bg-orange-50" onClick={() => setLocation("/routing")}>
